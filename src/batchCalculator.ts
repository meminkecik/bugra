...
export type BatchResult = {
  ...
  Vsa: {
    ...
    M5: number; M6: number; M7: number; Exact: number;
  };
  ...
  diff?: {
    ...
    M5: string; M6: string; M7: string; Exact: string;
  };
};
 ...
 */
export function calculateAllPresets(targetDepth?: number): { results: BatchResult[] } {
    ...
    if (result) {
      ...
      if (exp) {
        batchResult.expected = {
            M1: exp.Vsa_M1, M2: exp.Vsa_M2, M3: exp.Vsa_M3,
            M4: exp.Vsa_M4, M5: exp.Vsa_M5, M6: exp.Vsa_M6,
            M7: exp.Vsa_M7, Exact: exp.Exact
        };
        batchResult.diff = {
